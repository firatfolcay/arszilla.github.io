...
`joomscan` will gather us information regarding the Joomla installation.
...
to be susceptible to SQL injection (`CVE-2017-8917`). To use the SQL injection exploit we'll use 
[joomblah.py][joomblah.py].
...
`reddy@norealaddress.net` with the hashed password `$2y$10$DpfpYjADpejngxNh9GnmCeyIHCWpL97CVRnGeZsVJwR0kWFlfB1Zu`. 
...
To implement our reverse shell we'll use [pentestmonkey][pentestmonkey]'s [PHP Reverse Shell][PHP Reverse Shell]. We'll 
paste the script in `index.php` and use `nc` to catch the shell. After we establish a connection, we'll spawn a proper 
shell using `python` and use [LinEnum][LinEnum] to gather more info on the system we're working with.
